package com.alnaiyr.uinterface.listeners;
...
/**
 * Rewrite of moues opver area, that is too complicated and not enough flexible
 * 
 * @author IgoЯ
 * @version 1.0
 *          <p>
...
public class MouseOverArea extends AbstractComponent {
    ...
    private int state = MouseOverArea.NORMAL;
	...
	this(container, image, x, y, image.width(), image.height());
	...
	if (!over) {
	    ...
	    mouseUp = true;
	    if (state != MouseOverArea.MOUSE_OVER) {
		if (mouseOverSound != null) {
		    mouseOverSound.play();
		}
		currentImage = mouseOverImage;
		currentColor = mouseOverColor;
		state = MouseOverArea.MOUSE_OVER;
	    }
	}
