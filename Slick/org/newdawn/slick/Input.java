...
public class Input {
	...
	protected BooleanStorer[] pressed = new BooleanStorer[1024];
	 ...
	 * called for the specified key, but is non destructive
	 * 
	 * @param code
	...
	public boolean isKeyPressed(int code) {
		...
		//
		// return false;
		return pressed[code].getValue().booleanValue();
	}
	...
	public void poll(int width, int height) {
		...
		// Iterator<ControlledInputReciever> allStarts =
		// allListeners.iterator();
		...
		while (Mouse.next()) {
			if (Mouse.getEventButton() >= 0) {
				if (Mouse.getEventButtonState()) {
					...
					pressedY = (int) (yoffset + (height - Mouse.getEventY())
							* scaleY);
			...
			if (listener.isAcceptingInput()) {
				switch (index) {
					...
					default:
						...
						listener.controllerButtonPressed(controllerIndex, index
								- BUTTON1 + 1);
						break;
				}
				if (consumed) {
