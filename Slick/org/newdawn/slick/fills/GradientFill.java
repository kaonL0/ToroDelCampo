...
public class GradientFill implements ShapeFill {
	...
	private PlanVector start;
	/** The end poisition of the gradient */
	private PlanVector end;
	 ...
	 * @param local True if the gradient is defined in local shape coordinates
	 */
	public GradientFill(PlanVector start, Color startCol, PlanVector end, Color endCol, boolean local) {
	 ...
	 */
	public void setStart(PlanVector start) {
		this.start = new Vector2f(start);
	}
	 ...
	 */
	public void setEnd(PlanVector end) {
		this.end = new Vector2f(end);
	}
	...
	public Color colorAt(float x, float y) {
		...
		ua /= denom;
		float ub = (dx1 * (start.y() - y)) - (dy1 * (start.x() - x));
		ub /= denom;
		float u = ua;
		if (u < 0) {
			u = 0;
