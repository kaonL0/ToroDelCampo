...
import com.java.modplayer.sound.Module;
import com.java.modplayer.sound.tracker.FastTracker2;
import com.java.modplayer.sound.tracker.ProTracker;
import com.java.modplayer.sound.tracker.ScreamTracker3;
...
public class OpenALMODPlayer {
	...
	private int[] data = new int[OpenALMODPlayer.sectionSize * 4];
	/** The byte buffer passed to OpenAL containing the section */
	private IntBuffer bufferData = BufferUtils
			.createIntBuffer(OpenALMODPlayer.sectionSize * 4);
	/**
	...
	private com.java.modplayer.sound.Module module;
	 ...
	 *             The input stream to read the music from
	 */
	public void play(InputStream in, boolean loop, boolean start)
			throws IOException {
	 ...
	 *             The input stream to read the music from
	 */
	public void play(int source, InputStream in, boolean loop, boolean start)
			throws IOException {
		if (!soundWorks) {
			return;
		}
	 ...
	 */
	public static com.java.modplayer.sound.Module loadModule(InputStream in)
			throws IOException {
		com.java.modplayer.sound.Module module;
		DataInputStream din;
		...
		if (FastTracker2.isSupportedModule(xm_header)) {
			module = FastTracker2.loadModule(xm_header, din);
		} else {
			s3m_header = new byte[96];
			System.arraycopy(xm_header, 0, s3m_header, 0, 60);
			din.readFully(s3m_header, 60, 36);

			if (ScreamTracker3.isSupportedModule(s3m_header)) {
				module = ScreamTracker3.loadModule(s3m_header, din);
			} else {
				mod_header = new byte[1084];
				System.arraycopy(s3m_header, 0, mod_header, 0, 96);
				din.readFully(mod_header, 96, 988);
				module = ProTracker.loadModule(mod_header, din);
			}
		}
		din.close();
	...
	public boolean stream(int bufferId) {
		int frames = OpenALMODPlayer.sectionSize;
		boolean reset = false;
		boolean more = true;
