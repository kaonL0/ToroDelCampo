...
public class TextureImpl implements Texture {
	...
	public static void bindNone() {
		...
		TextureImpl.GL.glDisable(SGL.GL_TEXTURE_2D);
	}
	...
	public void bind() {
		if (TextureImpl.lastBind != this) {
			TextureImpl.lastBind = this;
			TextureImpl.GL.glEnable(SGL.GL_TEXTURE_2D);
	...
	public byte[] getTextureData() {
		ByteBuffer buffer = BufferUtils.createByteBuffer((hasAlpha() ? 4 : 3)
				* texWidth * texHeight);
		bind();
		TextureImpl.GL.glGetTexImage(SGL.GL_TEXTURE_2D, 0,
				hasAlpha() ? SGL.GL_RGBA : SGL.GL_RGB, SGL.GL_UNSIGNED_BYTE,
				buffer);
