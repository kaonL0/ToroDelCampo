...
public class BigImage extends Image {
	 ...
	 *             Indicates we were unable to locate the resource
	 */
	public BigImage(final String ref, final int filter, final int tileSize)
			throws SlickException {
	 ...
	 *            The image filter to apply (@see #Image.FILTER_NEAREST)
	 */
	public BigImage(final LoadableImageData data, final ByteBuffer imageBuffer,
			final int filter) {
	 ...
	 */
	public BigImage(final LoadableImageData data, final ByteBuffer imageBuffer,
			final int filter, final int tileSize) {
	 ...
	 *             Indicates we were unable to locate the resource
	 */
	private void build(final String ref, final int filter, final int tileSize)
			throws SlickException {
		try {
	...
	private void build(final LoadableImageData data,
			final ByteBuffer imageBuffer, final int filter, final int tileSize) {
	...
	public void draw(final float x, final float y, final float width,
		...
		final float sx = width / realWidth;
		final float sy = height / realHeight;
		...
		for (int tx = 0; tx < xcount; tx++) {
			...
			for (int ty = 0; ty < ycount; ty++) {
				final Image image = images[tx][ty];

				image.draw(xp, yp, image.width(), image.height(), filter);
	...
	public void draw(final float x, final float y, final float x2,
		...
		final Image subImage = getSubImage((int) srcx, (int) srcy, srcwidth,
				srcheight);
	...
	public void drawEmbedded(final float x, final float y, final float width,
		...
		for (int tx = 0; tx < xcount; tx++) {
			...
			for (int ty = 0; ty < ycount; ty++) {
				...
				image.drawEmbedded(xp + x, yp + y, image.width(),
						image.height());
	...
	public void drawFlash(final float x, final float y, final float width,
		...
		for (int tx = 0; tx < xcount; tx++) {
			...
			for (int ty = 0; ty < ycount; ty++) {
				...
				image.drawFlash(xp, yp, image.width(), image.height());
	...
	public Image getSubImage(final int x, final int y, final int width,
		...
		for (int xt = 0; xt < xcount; xt++) {
			...
			for (int yt = 0; yt < ycount; yt++) {
				final Image current = images[xt][yt];

				final int xp2 = (int) (xp + current.width());
				final int yp2 = (int) (yp + current.height());
				...
				if (targetWidth > 0 && targetHeight > 0) {
					final Image subImage = current.getSubImage(
							(int) (targetX1 - xp), (int) (targetY1 - yp),
							targetX2 - targetX1, targetY2 - targetY1);
					foundStart = true;
