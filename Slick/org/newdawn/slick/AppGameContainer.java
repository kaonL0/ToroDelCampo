...
public class AppGameContainer extends GameContainer {
	...
	public void setMouseCursor(Image image, int hotSpotX, int hotSpotY)
			throws SlickException {
		try {
			Image temp = new Image(get2Fold(image.width()),
					get2Fold(image.height()));
			Graphics g = temp.getGraphics();

			ByteBuffer buffer = BufferUtils.createByteBuffer(temp.width()
					* temp.height() * 4);
			...
			Cursor cursor = CursorLoader.get().getCursor(buffer, hotSpotX,
					hotSpotY, temp.width(), image.height());
			Mouse.setNativeCursor(cursor);
		} catch (Exception e) {
			Log.error("Failed to load and apply cursor.", e);
		}
