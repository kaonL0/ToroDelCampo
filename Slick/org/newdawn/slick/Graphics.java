...
public class Graphics {
	...
	public void drawLine(float x1, float y1, float x2, float y2) {
		...
		if (LSR.applyGLLineFixes()) {
			if (x1 == x2) {
				...
				fillRect(x1 - lineWidth / 2.0f, y1 - lineWidth / 2.0f, x2 - x1
						+ lineWidth + step, lineWidth + step);
				return;
			}
		}
	...
	public void fillRect(float x, float y, float width, float height,
		...
		int cols = (int) Math.ceil(width / pattern.width()) + 2;
		int rows = (int) Math.ceil(height / pattern.height()) + 2;
		...
		for (int c = 0; c < cols; c++) {
			for (int r = 0; r < rows; r++) {
				pattern.draw(c * pattern.width() + x - offX,
						r * pattern.height() + y - offY);
			}
		}
		postdraw();
	...
	public void drawArc(float x1, float y1, float width, float height,
		...
		for (int a = (int) start; a < (int) (end + step); a += step) {
			...
			float x = (float) (cx + FastTrig.cos(Math.toRadians(ang)) * width
					/ 2.0f);
			float y = (float) (cy + FastTrig.sin(Math.toRadians(ang)) * height
					/ 2.0f);
	...
	public void drawImage(Image image, float x, float y, float srcx,
		...
		drawImage(image, x, y, x + image.width(), y + image.height(), srcx,
				srcy, srcx2, srcy2);
	}
	...
	public void copyArea(Image target, int x, int y) {
		...
		GL.glCopyTexImage2D(SGL.GL_TEXTURE_2D, 0, format, x, screenHeight
				- (y + target.height()), target.getTexture().getTextureWidth(),
